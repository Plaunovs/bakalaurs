"""
run_deepface.py — DeepFace sistēma (MTCNN detektors + VGG-Face 4096D).
...
Palaišana:
    ...
    python DeepFace/run_deepface.py --run 1 [--dataset Dataset]
"""
...
def _is_no_face_error(exc: Exception) -> bool:
    msg = str(exc).lower()
    return any(kw in msg for kw in ("face", "detect", "cannot", "no face"))
...
def encode_one(img_path: str):
    ...
    try:
        ...
        if not result:
            return None
        return l2n(np.array(result[0]["embedding"], dtype=np.float32))
...
def _best_match_sim(enc: np.ndarray, gallery: dict) -> tuple[str, float]:
    """Atgriež (person, max_cosine_sim); abi vektori jau L2-normalizēti."""
